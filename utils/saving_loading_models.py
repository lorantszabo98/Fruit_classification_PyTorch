...
def load_model(load_directory, model, number_of_epochs, additional_text='', augmentation=''):
    ...
    model.load_state_dict(torch.load(os.path.join(load_directory, f"{model_name}{additional_text}_model_epochs_{number_of_epochs}{augmentation}_7.pth"), map_location=torch.device('cpu')))